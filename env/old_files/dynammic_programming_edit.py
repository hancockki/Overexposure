    ...
    for numSeeds in range(0,k): #bottom up DP
        ...
        for node, j in zip(nodes, range(0,i)): 
            if j == 0 and numSeeds == 0: #first entry
                ...
                lastEntryUp = storePayoff[numSeeds-1][j] #up entry
                storePayoff[numSeeds][j] = max(nextGuess, lastEntry, lastEntryUp, last) #take biggest
                if storePayoff[numSeeds][j] == nextGuess:
                    print("ours was better")
                    table = storeSeeds[numSeeds-1][j-1][:]
                    table.append(node)
                    storeSeeds[numSeeds][j] = table
                elif storePayoff[numSeeds][j] == lastEntry:
                    print("last entry left")
                    nextList = storeSeeds[numSeeds][j-1][:]
                    storeSeeds[numSeeds][j] = nextList
                    #storePayoff[numSeeds][j] -= 1
                elif storePayoff[numSeeds-1][j] == lastEntryUp:
                    print("last entry up")
...
def computePayoff(G, i, k, source):
    ...
    for numSeeds in range(0,k): #bottom up DP
        nodes = list(reversed(list((nx.topological_sort(tree))))) #consider nodes in reverse topological sort order
        for node, j in zip(nodes, range(0,i)): 
            if j == 0 and numSeeds == 0: #first entry
                #breakpoint()
                storeSeeds[numSeeds][j] = [node]
...
def DP_Improved(G, k):
   ...
   # print("root is", nodes[0])
    ...
    storePart = [[0] * (k+1) for _ in range(num_children)] #memoization table
    ...
    for p in part: #get each partition of the seeds
        ...
        for i, subgraph in zip(range(0,num_children), subgraph_list): #pick the correct number of seeds for each subgraph
            ...
            if p[i] != 0:
                if storePart[i][p[i]] != 0: #memoization
                    amount += storePart[i][p[i]]
                    continue
                else:
...
def college_Message():
    ...
    makeMatrix(G_College_Msg, G_College_Msg.number_of_nodes())
...
def testOriginaltoCluster(n, c, k):
    ...
    print("cluster dict:", clusterDict)
    print("rej node dict", rejectingNodeDict)
    test1 = tryDP(G_cluster, G_cluster.number_of_nodes(), k) #try regular DP
...
def main():
    G = testOriginaltoCluster(60, 0.6, 6)
    #testRandomCluster()
    #G = college_Message()
